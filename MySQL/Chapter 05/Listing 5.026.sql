...
-- 【概要】5.025のさらに改善されたパターン。スパイスフィルタをサブクエリのWHERE句内に
--         移動することで、スパイスのみを含む結果セットをLEFT JOINする明確な構造。
--         スパイスを一切使わないメインコースレシピも正しくCOUNT=0で取得できる。

USE RecipesSample;

-- 推奨例: サブクエリ内のWHEREでスパイス絞り込みを行い、LEFT JOINでスパイスなしも保持
...
FROM (Recipe_Classes
  ...
  LEFT OUTER JOIN
  -- スパイスカテゴリのみをWHEREで事前フィルタしたサブクエリ
